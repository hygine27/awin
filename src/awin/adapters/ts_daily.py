...
读取 `stg.ts_daily` 的 T-1 及更早历史日线，用于 style_profile 的
成交额、动量、波动率和回撤计算。
"""
...
class TsDailyAdapter(DbBackedAdapter):
    """Load historical T-1 daily bars from `stg.ts_daily` for style profiling."""

    source_name = "ts_daily"
    ...
    def load_rows_with_health(self, trade_date: str) -> tuple[list[dict[str, object]], SourceHealth]:
        ...
        health = SourceHealth(source_name=self.source_name, source_status="ready" if rows else "degraded", detail="ok" if rows else "no_daily_rows")
        return rows, health
